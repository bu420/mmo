...
void mmo_player_new(mmo_player_t *player, mmo_server_t *server,
  ...
  {
    mmo_telnet_negotiation_state_t *negotiation_state =
        mmo_malloc(sizeof(mmo_telnet_negotiation_state_t));
    mmo_telnet_negotiation_state_new(negotiation_state);

    mmo_player_state_t state;
    state.ctx    = negotiation_state;
    state.free   = mmo_telnet_negotiation_state_free;
    state.update = mmo_telnet_negotiation_state_update;
    state.render = mmo_telnet_negotiation_state_render;
...
static void mmo_handle_new_and_old_clients(mmo_game_t *game,
  ...
  for (size_t i = 0; i < server->events.removed_clients.num_elems; i += 1) {
    mmo_player_t *player = mmo_player_arr_find(
        &game->players, find_player, &server->events.removed_clients.elems[i]);
