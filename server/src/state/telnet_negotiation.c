#include <mmo/state/telnet_negotiation.h>
...
bool mmo_has_received_complete_telnet_command(const mmo_char_arr_t *bytes) {
  ...
  case 0:
    return bytes->num_elems >= 4 * sizeof(int32_t);
...
void mmo_handle_telnet_command(mmo_char_arr_t *bytes, mmo_client_t *client,
                               mmo_server_t *server) {
  assert(bytes);
  assert(client);
  assert(server);
  assert(bytes->elems[0] == (char)0xff && "First byte must be IAC (0xff).");
...
void mmo_telnet_negotiation_state_free(void *) {}

void mmo_telnet_negotiation_state_update(void *ctx, mmo_client_handle_t handle,
                                         mmo_server_t *server,
                                         mmo_char_arr_span_t *) {
  mmo_telnet_negotiation_state_t *state = ctx;
}

void mmo_telnet_negotiation_state_render(void *, mmo_screen_buf_t *) {}
