...
void mmo_string_split(mmo_char_span_t text, char delimiter,
                      mmo_char_span_list_t *out) {
  assert(out);
...
void mmo_string_justify_and_hyphenate(mmo_char_span_t in, int width,
  ...
  while (word) {
    ...
    while (word && (size_t)pos + word->data.num_elems <= (size_t)width) {
      /* +1 for the required minimum of one following space. Used for
       * calculation. */
    ...
    if (word && words_on_line.num_elems < 2 && num_spaces >= 3) {
      /* Make copy of base, add hyphen and add it to array of words that fit on
       * line. */
      ...
      /* Copy current word view, offset it to point to remainder and insert new
         view into list of all words to be processed next iteration. */
    ...
    for (size_t i = 0; i < words_on_line.num_elems; i += 1) {
      ...
      if (i == words_on_line.num_elems - 2) {
        ...
        num_spaces = (int)round((float)num_total_spaces /
                                ((float)words_on_line.num_elems - 1));
