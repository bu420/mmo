...
void mmo_string_justify_and_hyphenate(mmo_char_span_t in, int width,
    ...
    while (word) {
        ...
        if (word && words_on_line.num_elems < 2 && num_spaces >= 3) {
            /* Make copy of base, add hyphen and add it to array of words that
             * fit on line. */
            ...
            /* Copy current word view, offset it to point to remainder and
               insert new view into list of all words to be processed next
               iteration. */
