...
void mmo_ansi_move_cursor(int x, int y, mmo_char_arr_t *out) {
    ...
    out->num_elems = (size_t)snprintf(out->elems, out->num_elems, "\x1b[%d;%dH", x, y);
}
...
void mmo_screen_buf_new(mmo_screen_buf_t *buf, int width, int height) {
    ...
    memset(buf->cells_modified_flags.elems, false, buf->cells_modified_flags.num_elems);
}
...
static void mmo_cell_color_to_str(const mmo_cell_color_t *color, int layer, mmo_char_arr_t *out) {
    assert(color);
    assert(out);
...
static bool mmo_cell_color_equals(const mmo_cell_color_t *a, const mmo_cell_color_t *b) {
